...
| `typing_interval_seconds` | integer | `5` | At most ONE `sendChatAction` per chat every N seconds, regardless of how often omniagent requests typing (core enqueues one every 5 s). Excess requests are dropped, never queued. `0` (or an empty value) disables the typing signal entirely; invalid/negative values fall back to the default |
